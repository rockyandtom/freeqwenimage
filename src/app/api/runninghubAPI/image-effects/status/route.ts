...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('图像特效任务状态响应:', statusData)
    ...
    if (statusData.code === 0 && statusData.data) {
      ...
      if (status === 'SUCCESS' || status === 'COMPLETED') {
        ...
        if (outputResponse.ok) {
          ...
          console.log('图像特效任务结果响应:', outputData)
        ...
        return NextResponse.json({
          success: false,
          error: 'Image effects task failed on RunningHub'
        }, { status: 500 })
    ...
    console.error('Image effects status check error:', error)
    return NextResponse.json(
      { 
        error: 'Failed to check image effects task status',
