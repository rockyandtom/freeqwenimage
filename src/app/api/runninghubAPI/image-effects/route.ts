...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType && contentType.includes('multipart/form-data')) {
      ...
      console.log('提交图像特效任务到RunningHub:', requestData)
      ...
      if (submitResult.code === 0 && submitResult.data && submitResult.data.taskId) {
        ...
        // 任务提交成功，返回任务ID让前端轮询
        console.log(`图像特效任务提交成功，taskId: ${taskId}`)
        ...
        throw new Error('Failed to submit image effects task to RunningHub')
      }
    } else {
    ...
    console.error('Image Effects API Error:', error)
    return NextResponse.json(
      { 
        error: 'Failed to process image effects',
...
export async function GET(request: NextRequest) {
  ...
  if (debug === 'true') {
    try {
      console.log(`调试模式：查询图像特效任务状态 ${taskId}`)
